...
public struct KotlinGenerator: Generator {
    ...
    public init(representation: OutputRepresentation, packageName: String, filteringOutStaticStrings: Bool, version: String) {
        let representationToUse: OutputRepresentation? = {
            filteringOutStaticStrings ? representation.filter({ $0.hasParameters }):representation
        }()

        self.code = {
            guard let representationToUse = representationToUse else { return "" }
            return KotlinGenerator.root(representationToUse, packageName: packageName, version: version)
        }()
    }
    ...
    private static func root(_ representation: OutputRepresentation, packageName: String, version: String) -> String {
        ...
        internal object LocalizedStrings {
          ...
          \(representation.languages.sorted(by: >).map({ language in
          ...
          private fun getCurrentLang(): SupportedLanguage {
            ...
            return SupportedLanguage.values().firstOrNull { locale.startsWith(it.identifier) } ?:
              SupportedLanguage.values().firstOrNull { locale.startsWith(it.identifier.split("_")[0]) }  ?:
              SupportedLanguage.\(representation.defaultLanguage.uppercased().sanitized)
          }
